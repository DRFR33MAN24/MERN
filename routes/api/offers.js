...
router.post("/", auth, (req, res) => {
  ...
  // OfferProvider.findOne({where:{name:"cpalead"}})
  //.then(p=>console.log(p.lastUpdate))
  // cpalead
  // kiwi
  ...
  axios
    ...
    .then(res => {
      let offer_arr = []
      res.data.offers.map(({ title, description, link, previews, amount, conversion, country, device }) => {
        offer_arr.push({
          title: title,
  ...
  Offer.findAll({ where: { offer_type: offer_type, country: country, device: device } })
